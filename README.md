...
- **MIDI 片段模式**：垫可改为加载 .mid 文件，按住循环播放（BPM 30–300 可调，覆盖文件内速度），松开立即停止并自动补发 Note Off 防卡音
- **输入过滤**：音符选择框支持直接键入音名或号码（如 `c4`、`60`）实时过滤，回车提交
- 点击垫修改：模式、源音符、目标音符、输出通道（可跟随原通道）、静音；源音符冲突时状态栏警告
...
- **单实例运行**：全局只允许启动一个程序，重复启动时自动把已运行的窗口调到前台后退出
